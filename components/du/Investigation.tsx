...
export default function Investigation({ id, level, value, onChange }: {
  ...
  const [sourceId, setSourceId] = useState(inquiry.sources[0].id);
  ...
  return <div className={'investigation-layout showing-' + panel}>
    ...
    <section className="investigation-sources" aria-label="조사 자료">
      <p className="mobile-source-question">{question.prompt}</p>
      <div className="source-tabs" aria-label="자료 선택">{inquiry.sources.map(s => <button key={s.id} aria-pressed={s.id === source.id} onClick={() => setSourceId(s.id)}>{s.title}{answer.sources.includes(s.id) && <span aria-label="근거로 선택함"> ✓</span>}</button>)}</div>
    ...
    <section className="investigation-judgment" aria-label="내 판단"><div className="judgment-scroll">
      {questions.length > 1 && <nav className="question-tabs" aria-label="조사 질문">{questions.map((q, i) => <button key={q.id} aria-pressed={i === index} onClick={() => { onChange({ ...value, current: i }); setAnswerPart('choice'); }}>질문 {i + 1}{value?.answers?.[q.id]?.reviewed && inquiryCorrect(q, value.answers[q.id], level) ? ' ✓' : ''}</button>)}</nav>}
      ...
      {correct && index < questions.length - 1 && <button className="secondary next-question" onClick={() => { onChange({ ...value, current: index + 1 }); setPanel('sources'); setAnswerPart('choice'); }}>다음 질문 살펴보기</button>}
