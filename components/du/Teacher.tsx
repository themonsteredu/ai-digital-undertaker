...
import { ShieldCheck, LogOut, Plus, ArrowLeft, ArrowRight, Expand, Trash2, RefreshCw } from 'lucide-react';
...
import { CASES, SLIDES, riskItems, stepsFor } from '@/lib/du/content';
import { Gallery, StampCanvas } from './Finish';
function Slides() { const [n, setN] = useState(0); const ref = useRef<HTMLDivElement>(null); useEffect(() => { const key = (e: KeyboardEvent) => { if (e.key === 'ArrowRight')
    setN(v => Math.min(9, v + 1)); if (e.key === 'ArrowLeft')
    setN(v => Math.max(0, v - 1)); }; window.addEventListener('keydown', key); return () => window.removeEventListener('keydown', key); }, []); return <><div className="slides-frame" ref={ref}><img src={'/art/' + SLIDES[n][2] + '.png'} alt="디지털 기록과 개인정보 보호 활동"/><h2>{SLIDES[n][0]}</h2><div className="slide-arrows"><button onClick={() => setN(Math.max(0, n - 1))} aria-label="이전 슬라이드" disabled={!n}><ArrowLeft /></button><button onClick={() => setN(Math.min(9, n + 1))} aria-label="다음 슬라이드" disabled={n === 9}><ArrowRight /></button></div></div><div className="slides-nav"><span>{n + 1} / 10</span><button onClick={() => ref.current?.requestFullscreen().catch(() => { })}><Expand size={18}/>전체화면</button></div><div className="presenter-notes"><b>발표자 메모</b><p>{SLIDES[n][1]}</p></div></>; }
...
export default function Teacher({ token, onExit }: {
    ...
    <TabsContent value="gallery"><Gallery data={board}/></TabsContent><TabsContent value="slides"><Slides /></TabsContent></Tabs>
